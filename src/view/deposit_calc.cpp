    ...
    ui(new Ui::deposit_calc)
{
    ...
    ui->tableWidget->setHorizontalHeaderLabels(QStringList() << "Сумма пополнения");
    ui->tableWidget->setColumnWidth(0, 265);

    ui->tableWidget_2->setRowCount(0);
    ui->tableWidget_2->setColumnCount(1);
    ui->tableWidget_2->setHorizontalHeaderLabels(QStringList() << "Сумма списания");
    ui->tableWidget_2->setColumnWidth(0, 265);

    connect(ui->horizontalSlider, QSlider::valueChanged, this, deposit_calc ::on_horizontalSlider_actionTriggered);
}
...
void deposit_calc::on_add_money_clicked() {
    QTableWidgetItem *item = new QTableWidgetItem(QString::number(ui->spinBox_add->value()));
...
void deposit_calc::on_add_money_2_clicked() {
    QTableWidgetItem *item = new QTableWidgetItem(QString::number(ui->spinBox_sub->value()));
